...
const Navbar = () => {
  ...
  const getIcon = (label) => {
    ...
    // New pages
    if (l.includes('hierarchy'))   return 'bi-diagram-3';
    if (l.includes('task report')) return 'bi-list-check';
    if (l.includes('weekly'))      return 'bi-calendar-check';
  ...
  const getMenuItems = () => {
    ...
    // Shared new links
    const hierarchy = { label: 'Hierarchy', path: '/hierarchy' };
    ...
    // dashboard is the TL dashboard, not the generic one.
    if (user?.role === 'tl')
      return [
        { label: 'Dashboard', path: '/tl-dashboard' },
        { label: 'Task Report', path: '/task-report' },
