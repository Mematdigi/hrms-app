...
const Dashboard = () => {
   ...
   // ==============================
   // 4b. Leave Eligibility (Permanent + Full-Time)
   // ==============================
   // Only PERMANENT + FULL-TIME employees can apply for every leave type
   // (short / half / casual / sick). Everyone else is limited to UNPAID only.
   //
   // Primary source is the logged-in `user`; falls back to the matching record
   // in allEmployees. Adjust field names below to match your employee schema.
   ...
   const isFullTime  = empStatus.includes('full'); // matches "full-time", "full time", "fulltime"

   // Master flag: can this user apply for all leave types?
   const canApplyAllLeaves = isPermanent && isFullTime;
   // If you want HR/managers always allowed regardless of their record, use:
   // const canApplyAllLeaves = isHR || (isPermanent && isFullTime);
   ...
   const handleOpenLeaveModal = () => {
      if (!canApplyAllLeaves) {
         // Restricted users → Full Day → Unpaid Leave only
         setLeaveType('full');
         setSelectedLeaveCategory('unpaid');
   ...
   const handleSelectLeaveType = (type) => {
      // Restricted users cannot use Short or Half Day at all
      if (!canApplyAllLeaves && (type === 'short' || type === 'half')) return;
   ...
   const handleSubmitLeave = async () => {
      try {
         // ── Eligibility guard: only permanent + full-time can go beyond unpaid ──
         if (!canApplyAllLeaves && (leaveType !== 'full' || selectedLeaveCategory !== 'unpaid')) {
            alert('⚠️ Only Permanent + Full-Time employees can apply for this leave type. You can apply for Unpaid Leave only.');
            return;
         }
      ...
      <div className="hr-page">
         ...
         <Modal show={showLeaveModal} onHide={() => setShowLeaveModal(false)} centered className="leave-modal">
            ...
            <Modal.Body className="pt-2">
               ...
               {/* ── Restriction banner: non permanent/full-time employees ── */}
               {!canApplyAllLeaves && (
                  <div className="alert alert-soft-red mb-3 py-2 small">
                     <i className="bi bi-info-circle me-2"></i>
                     As per company policy, only <strong>Permanent + Full-Time</strong> employees can apply for Short, Half Day, Casual or Sick leave. You can apply for <strong>Unpaid Leave</strong> only.
                  </div>
               )}
               ...
               <div className="d-flex gap-2 mb-3">
                  <div
                     className={`leave-type-card flex-fill ${leaveType === 'short' ? 'active' : ''} ${(!canApplyAllLeaves || shortLeavesTaken >= shortLeavesLimit) ? 'disabled' : ''}`}
                  ...
                  <div
                     className={`leave-type-card flex-fill ${leaveType === 'half' ? 'active' : ''} ${(!canApplyAllLeaves || isHalfDayLimitReached) ? 'disabled' : ''}`}
            ...
            <Modal.Footer className="border-0 pt-0">
               ...
               <Button
                  ...
                  disabled={
                     (!canApplyAllLeaves && (leaveType !== 'full' || selectedLeaveCategory !== 'unpaid')) ||
                     (leaveType === 'short' && shortLeavesTaken >= shortLeavesLimit) ||
                     (leaveType === 'half'  && isHalfDayLimitReached) ||
