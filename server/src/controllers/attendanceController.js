...
class AttendanceController {
  ...
  requestEarlyCheckout = async (req, res) => {
    try {
      ...
      // 🔔 Notify HR/Admin + the employee's TL of the pending request (added)
      const requester = await User.findById(employeeId).select('firstName lastName teamLead_id');
      if (requester) {
        await notifyByRoles(['hr', 'admin'], {
          sender:   employeeId,
          type:     'early_checkout_requested',
          title:    `Early Checkout Request — ${requester.firstName} ${requester.lastName}`,
          message:  `${requester.firstName} ${requester.lastName} requested an early checkout. Reason: ${reason.trim()}`,
          refId:    attendance._id,
          refModel: 'Attendance',
        });
        if (requester.teamLead_id) {
          await createNotification({
            recipient: requester.teamLead_id,
            sender:    employeeId,
            type:      'early_checkout_requested',
            title:     `Early Checkout Request — ${requester.firstName}`,
            message:   `Your team member ${requester.firstName} requested an early checkout. Reason: ${reason.trim()}`,
            refId:     attendance._id,
            refModel:  'Attendance',
          });
        }
  ...
  approveEarlyCheckout = async (req, res) => {
    try {
      ...
      // 🔔 Notify the employee of the decision (added — this flow sent no notification before)
      await createNotification({
        recipient: attendance.employee,
        sender:    hrId,
        type:      approved ? 'early_checkout_approved' : 'early_checkout_rejected',
        title:     approved ? 'Early Checkout Approved ✅' : 'Early Checkout Rejected ❌',
        message:   approved
          ? 'Your early checkout request has been approved. You have been checked out.'
          : 'Your early checkout request was rejected. Please continue your working hours.',
        refId:     attendance._id,
        refModel:  'Attendance',
      });
